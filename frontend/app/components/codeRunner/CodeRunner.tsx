...
import { ExecuteResult } from '@/dtos/execute';
...
export default function CodeRunner({tasks}: CodeRunnerProps) {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-700">
      ...
      <div>
        <Editor loading={<div className="font-semibold text-white">Loading...</div>} options={{ padding: { top: 10 } }} theme="vs-dark" height="60vh" onMount={handleEditorDidMount} defaultLanguage="python" defaultValue="print(input())" />
      </div>
      <div className="pl-8 pt-4 flex-auto">
        {!isExecuting ? <Output result={executeResult}/>
        : <LoadingCircle/>}
      </div>
    </div>
  )
